...
# --- Sudoers Command to Toggle ---
@app.on_message(filters.command("autoleave") & filters.user(config.SUDOERS))
                    ...
                    async for i in client.get_dialogs():
                        if i.chat.type in [ChatType.SUPERGROUP, ChatType.GROUP, ChatType.CHANNEL]:
                            ...
                            if i.chat.id != config.LOGGER_ID and i.chat.id != -1003809966719:
                                if left == 20: # Spam prevent karne ke liye limit
                                    break
...
async def auto_end():
    while True:
        ...
        if not ender:
            continue
        for chat_id in list(autoend): # list() used to avoid dictionary size change error
